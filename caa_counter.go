...
// time a session is issued the CAACounter is incremnented and a value is returned to
...
// the CAACounter and if it is within the delta then the session can be considered
// valid. The CAACounter can be locked and unlocked at will with out a need to update
...
type CAACounter int64

func NewCounter() *CAACounter {
	return new(CAACounter)
}

// Locks CAA to prevent validation of session CAA's.
func (caa *CAACounter) Lock() {
	*caa = -caa.abs()
}

// Unlocks CAA to allow validation of session CAA's.
func (caa *CAACounter) Unlock() {
	*caa = caa.abs()
}

func (caa CAACounter) IsLocked() bool {
	return caa < 0
}
...
func (caa CAACounter) IsValid(s SessionCAA, delta int64) bool {
...
func (caa *CAACounter) Revoke(n int64) {
	if !caa.HasIssued() {
		return
	}
...
func (caa *CAACounter) Issue() SessionCAA {
	defer caa.step(1)
...
// locked.
func (caa CAACounter) HasIssued() bool {
	return caa != 0
}

func (caa CAACounter) abs() CAACounter {
	return CAACounter(abs(int64(caa)))
}

func (caa *CAACounter) step(n int64) {
	if caa.IsLocked() {
		caa.decrement(n)
	} else {
		caa.increment(n)
...
func (caa *CAACounter) increment(n int64) {
	*caa += CAACounter(n)
}
func (caa *CAACounter) decrement(n int64) {
	*caa -= CAACounter(n)
}
